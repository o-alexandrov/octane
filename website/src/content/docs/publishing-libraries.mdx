...
`source` is the one marker a workspace or linked package may need. A repository that
pins Octane once in a shared toolkit manifest and consumes it transitively can claim a
package as Octane source without re-declaring a version range it does not own:
...
It applies only to packages outside `node_modules`; a published package installed under
`node_modules` is recognized by its declared `octane` dependency as before.
